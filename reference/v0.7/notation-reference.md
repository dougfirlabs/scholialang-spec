# Scholia v0.7 — Notation Reference

**Status:** Proposed candidate (not ratified), derived from `reference/v0.7/atoms_index.yaml` (v0.7).
**Spec:** see [`docs/scholia/SCHOLIA_v0.7_SPEC.md`](../../docs/scholia/SCHOLIA_v0.7_SPEC.md) for the full v0.7 specification.

This document lists the 35 atoms in the Scholia v0.7 closed set with their attributes, body conventions, examples, and applicable validator rules. One section per atom; atoms within a category are alphabetical.

> **Generated file.** Edit `atoms_index.yaml` and regenerate via `python scripts/notation_reference_gen.py --index reference/v0.7/atoms_index.yaml --out reference/v0.7/notation-reference.md`. The consistency test in `tests/test_spec_consistency.py` enforces this on PR.
...
|---|---|---|---|
| `id` | string | no | trace-scoped local identifier (all versions). PROPOSED v0.7 per-kind override: REQUIRED on Map/Event/Task with a kind-matching prefix — see those kinds' own tables. |
| `canonical_id` | string | no | content address: sha256:<12 hex> over {kind, content.strip(), attrs}; provenance excluded. Parser-stamped; checked by canonical_id_well_formed. |
...
- **Reference** — [`<Implication>`](#implication) · [`<Map>`](#map) · [`<Print>`](#print) · [`<Reference>`](#reference) · [`<Storing>`](#storing)
- **Social** — [`<Handoff>`](#handoff) · [`<Question>`](#question) · [`<Review>`](#review)
- **Meta** — [`<Budget>`](#budget) · [`<Confidence>`](#confidence) · [`<Constraint>`](#constraint) · [`<Cost>`](#cost) · [`<Event>`](#event) · [`<EventRef>`](#eventref) · [`<Goal>`](#goal) · [`<Task>`](#task)
...
| `fingerprint` | algo:hex | no | content hash of the source span the Observation anchors to (FINGERPRINT.md); absence carries no claim. Present in implementation 0.7.2; recorded here as a documentation parity repair. |
...
| `for_goal` | string | no | DEPRECATED in v0.5 — use for_hyp. Still read-accepted in 0.7.x (deprecated, emits DeprecationWarning); removal deferred to a future major revision. |
...
|---|---|---|---|
| `id` | string | no | trace-scoped identifier. |
| `on` | string | no | id of the atom this Uncertainty annotates. |
...
**Category:** §2 Reference &nbsp;·&nbsp; **Slug:** `map` &nbsp;·&nbsp; **Body:** free-text (optional) — human explanation, not a second data payload.

| Attribute | Type | Required | Notes |
|---|---|---|---|
| `id` | string | yes | REQUIRED (overrides the optional base id): `Map_` followed by one or more ASCII letters, digits, or underscores. A collision involving a new-kind atom is an error anywhere in the trace. |
| `value_type` | string | integer | boolean | atom_ref | yes | declared homogeneous value type for every entry. Closed set; no heterogeneous or nested types in this revision. |
| `entries` | JSON object (XML attribute) / mapping (JSON, YAML, dict) | yes | possibly empty; unique nonempty string keys. `string` permits empty strings; `integer` requires a true integer (bool rejected) within ±(2^53−1); `boolean` requires bool exactly; `atom_ref` values are nonempty references resolving to a declared non-Step atom other than this Map. Floats, NaN, infinities, nulls, arrays, nested objects, nonstring keys, duplicate keys, and mixed types are rejected. XML output uses canonical JSON (sorted keys, compact separators, Unicode preserved) before XML escaping; JSON/YAML use a real mapping, never an encoded string. |
...
**Validator rules:** `semantic_id_shape`, `semantic_id_unique`, `semantic_unknown_field`, `semantic_children_empty`, `semantic_operators_list`, `map_required_fields`, `map_value_type`, `map_entries_shape`, `map_entries_typed`, `map_ref_resolves`, `canonical_id_well_formed` (see §4 of the canonical spec).
...
|---|---|---|---|
| `id` | string | no | trace-scoped identifier. |
| `on` | string | no | id of the atom this Confidence annotates. |
...
**Category:** §2 Meta &nbsp;·&nbsp; **Slug:** `event` &nbsp;·&nbsp; **Body:** free-text (REQUIRED, nonempty) — what was recorded.

| Attribute | Type | Required | Notes |
|---|---|---|---|
| `id` | string | yes | REQUIRED (overrides the optional base id): `Event_` followed by one or more ASCII letters, digits, or underscores. |
| `source` | string | yes | opaque producer namespace; not authentication evidence. |
| `occurrence_id` | string | yes | opaque ID unique within source for this occurrence. The pair (source, occurrence_id) identifies the occurrence; two Event atoms with the same pair in one trace are rejected even if their bodies match — transport deduplication belongs upstream of semantic insertion. |
| `event_type` | token | yes | nonempty classification token matching [A-Za-z][A-Za-z0-9_.-]*; producer-defined vocabulary, not a new atom kind. |
| `timestamp` | RFC3339 | no | explicit timezone required; naive or impossible date/time values rejected. Excluded from canonical identity, as existing timestamp provenance is excluded. |
| `for_task` | local-ref | no | resolves (forward references allowed) to a Task atom only. |
| `for_action` | local-ref | no | resolves to an Action atom only. |
| `for_goal` | local-ref | no | resolves to a Goal atom only. |
| `effect_ref` | local-ref | no | resolves to an Effect atom only; recording the link does not execute the effect. |
| `map_ref` | local-ref | no | resolves to a Map atom only. |
| `external_ref` | string | no | opaque external event/resource correlation; never interpreted as a local ID, dereferenced, used for execution, or treated as proof of identity/authority. |
...
**Validator rules:** `semantic_id_shape`, `semantic_id_unique`, `semantic_unknown_field`, `semantic_children_empty`, `semantic_operators_list`, `event_required_fields`, `event_type_token`, `event_timestamp_shape`, `event_occurrence_unique`, `semantic_ref_target_kind`, `canonical_id_well_formed` (see §4 of the canonical spec).
...
| `for` | string | no | id of the related local atom (Python field: for_ref). PROPOSED v0.7 coexistence note: may point at an Event atom where legacy reference resolution already permits it; EventRef never changes kind and gains no new automatic interpretation. |
...
**Category:** §2 Meta &nbsp;·&nbsp; **Slug:** `task` &nbsp;·&nbsp; **Body:** free-text (REQUIRED, nonempty) — the work obligation.

| Attribute | Type | Required | Notes |
|---|---|---|---|
| `id` | string | yes | REQUIRED (overrides the optional base id): `Task_` followed by one or more ASCII letters, digits, or underscores. |
| `status` | open | satisfied | unsatisfied | withdrawn | yes | descriptive claim, not enforcement or execution state. `open`: no semantic verdict; `satisfied`/`unsatisfied`: author's assertion the obligation was/was not met (evidence_ref REQUIRED); `withdrawn`: the obligation was withdrawn. A satisfied Task does NOT close a required Goal, substitute for a Finding/Concluding, excuse an Action without a recorded result, or declare a runtime task completed. Runtime states (queued, working, input_required, cancelled, expired, paused) are never legal here. A status change creates a new immutable Task atom/local ID and new canonical identity; accepted historical atoms are never mutated in place. |
| `for_goal` | local-ref | no | resolves (forward references allowed) to a Goal atom only. |
| `input_map` | local-ref | no | resolves to a Map atom only. |
| `output_map` | local-ref | no | resolves to a Map atom only. |
| `action_ref` | local-ref | no | resolves to an Action atom only. |
| `evidence_ref` | local-ref | no | resolves to an Observation, Evidence, Finding, or Concluding only. REQUIRED when status is satisfied or unsatisfied so the verdict has an explicit reference; validation checks shape/resolution/target kind, not the truth of the evidence. |
| `runtime_ref` | string | no | opaque correlator for a separately authorized runtime task; never creates or controls one, never interpreted as a local ID, and never proof of identity/authority. |
...
**Validator rules:** `semantic_id_shape`, `semantic_id_unique`, `semantic_unknown_field`, `semantic_children_empty`, `semantic_operators_list`, `task_required_fields`, `task_status_enum`, `task_evidence_required`, `semantic_ref_target_kind`, `canonical_id_well_formed` (see §4 of the canonical spec).
...
*Generated by `scripts/notation_reference_gen.py` from `reference/v0.7/atoms_index.yaml` (Scholia v0.7). If you found a drift between this doc and the canonical spec or the reference implementation, file an issue against `scholialang-spec`.*
