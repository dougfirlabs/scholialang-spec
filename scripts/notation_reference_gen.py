...
script emits one markdown section per atom (32 sections in v0.5),
...
def render(index: dict[str, Any]) -> str:
    ...
    lines: list[str] = []
    lines.append("# Scholia v0.5 — Notation Reference")
    lines.append("")
    lines.append("**Status:** Canonical, derived from "
                 f"`reference/atoms_index.yaml` (v{index.get('version')}).")
    lines.append("**Spec:** see "
                 f"[`docs/scholia/{index.get('spec')}`](../docs/scholia/{index.get('spec')}) "
                 "for the full v0.5 specification.")
    lines.append("")
    lines.append(
        f"This document lists the {index.get('total_atoms')} atoms in the "
        "Scholia v0.5 closed set with their attributes, body conventions, "
        "examples, and applicable validator rules. One section per atom; "
        "atoms within a category are alphabetical."
    )
    lines.append("")
    lines.append(
    ...
    lines.append(
        ...
        f"`reference/atoms_index.yaml` (Scholia v{index.get('version')}). "
