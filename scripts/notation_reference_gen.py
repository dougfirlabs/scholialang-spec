...
def render(index: dict[str, Any]) -> str:
    ...
    # Optional index keys (absent from the legacy v0.6 index, whose
    # rendered output must stay byte-identical): ``status`` marks a
    # proposed candidate catalog, ``index_label`` names the source
    # index, ``spec_link`` overrides the spec href for indexes that do
    # not sit directly under ``reference/``.
    status_label = (
        "Proposed candidate (not ratified)"
        if index.get("status") == "proposed" else "Canonical"
    )
    index_label = index.get("index_label", "reference/atoms_index.yaml")
    spec_link = index.get("spec_link", f"../docs/scholia/{index.get('spec')}")
    lines: list[str] = []
    lines.append(f"# Scholia v{version} — Notation Reference")
    lines.append("")
    lines.append(f"**Status:** {status_label}, derived from "
                 f"`{index_label}` (v{version}).")
    lines.append("**Spec:** see "
                 f"[`docs/scholia/{index.get('spec')}`]({spec_link}) "
    ...
    regen_command = (
        "python scripts/notation_reference_gen.py"
        if "index_label" not in index
        else f"python scripts/notation_reference_gen.py --index {index_label} "
             f"--out {index_label.rsplit('/', 1)[0]}/notation-reference.md"
    )
    lines.append(
        f"> **Generated file.** Edit `{index_label.rsplit('/', 1)[-1]}` "
        f"and regenerate via `{regen_command}`. The consistency test "
    ...
    lines.append(
        ...
        f"`{index_label}` (Scholia v{version}). "
